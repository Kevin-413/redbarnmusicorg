...
*/
	...
	public function init() {
		...
		$this->core = array(
			...
			'search' => array(
				...
				'parent' => 'archive'));
		$exclude = apply_filters('thesis_exclude_template_types', array());
		foreach ($wp_taxonomies as $slug => $data)
			if (empty($data->_builtin) && !in_array($slug, $exclude))
	...
	private function get_custom() {
		...
		return array(
			'templates' => $thesis->api->sort_by($templates, 'title', true),
			'ids' => $ids);
	}
	...
	public function custom_select() {
		...
		if (is_array($custom['templates']))
			foreach ($custom['templates'] as $id => $template)
				if (!empty($template['title']) && $template['title'][0] != '_')
	...
	public function editor_head() {
		...
		foreach ($css as $name => $href)
			echo "<link href=\"$href?ver={$thesis->version}\" rel=\"stylesheet\" />\n";
	}
	...
	public function editor($data) {
		...
		return
			...
			"$tab<h3><span id=\"template\" data-style=\"button\" title=\"". __('edit other templates or add a new template', 'thesis'). "\">{$this->template['title']}</span>$switch</h3>\n".
			$this->manager($depth).
	...
	private function manager($depth) {
		...
		return
			...
			"$tab\t\t<h4 class=\"manager_heading\">". __('Custom Templates', 'thesis'). " <span id=\"add_template\" data-style=\"button action\"  title=\"". __('add a new template', 'thesis'). "\">". __('add new', 'thesis'). "</span></h4>\n".
			$custom.
			"$tab\t</div>\n".
	...
	public function head($form) {
		...
		return
			...
			"$tab\t<button data-style=\"button save top-right\" id=\"save_options\" value=\"1\"><span data-style=\"dashicon big squeeze\">&#xf147;</span> ". $thesis->api->efn(__('Save HTML Head', 'thesis')). "</button>\n".
