...
class thesis_api {
	...
	public $hooks = array();		// list of available hooks on the current page
...
*/
	public function is_template($name = '') {
		global $thesis;
		return !empty($name) && !empty($thesis->skin->_template['title']) && $thesis->skin->_template['title'] === $name ? true : false;
	}
...
*/
	public function hook($hook, $args = '') {
		...
		$this->hooks[] = $hook;
		if (apply_filters('thesis_hooks', false) && is_user_logged_in() && current_user_can('manage_options') && (!is_admin() || (is_admin() && apply_filters('thesis_admin_hooks', false))))
			echo "<var class=\"thesis_hook\">$hook</var>\n";
...
/*
	The get_options method takes saved option data and combines it with
	default option data to determine the current state of options. Use
	this for FINAL decision-making purposes ONLY; do not feed data that
	has been sent through get_options() to the Form API.
	— $fields: an array of options in Thesis Options API Array Format (https://diythemes.com/thesis/rtfm/api/options/array-format/)
...
*/
	public function html_options($tags = false, $default_tag = false, $attributes = false, $group = false) {
...
*/
	...
	public function post($url, $body, $ssl = true) {
		if (empty($url) || empty($body)) return;
	...
	public function activate($key = false, $product = false, $name = false, $class = false, $url = false, $response_key = false) {
		global $thesis;
		update_option("{$class}_license_key", $key);
		...
		if (is_wp_error($response) || wp_remote_retrieve_response_code($response) !== 200) {
			$message = is_wp_error($response) ?
				...
				__('An unknown error occurred; please enter your license key again.', 'thesis');
			...
			if ($license_data->success === false) {
				switch($license_data->error) {
					case 'expired' :
						$message = sprintf(__('Your license key expired on %s.', 'thesis'), date_i18n(
							get_option('date_format'),
							strtotime($license_data->expires, current_time('timestamp'))));
						break;
					case 'disabled' :
					case 'revoked' :
						$message = __('Your license key has been disabled.', 'thesis');
						break;
					case 'missing' :
						$message = __('Invalid license key.', 'thesis');
						break;
					case 'invalid' :
					case 'site_inactive' :
						$message = __('Your license is not active for this URL.', 'thesis');
						break;
					case 'item_name_mismatch' :
						$message = sprintf(__('This appears to be an invalid license key for %s.', 'thesis'), $this->update['item_name']);
						break;
					case 'no_activations_left' :
						$message = __('Your license key has reached its activation limit.', 'thesis');
						break;
					default :
						$message = __('An unknown error occurred, please enter your license key again.');
						break;
				}
				if (!empty($message)) {
					wp_redirect(add_query_arg(array(
		...
		if ($license_data && isset($license_data->license)) {
			update_option("{$class}_license_key_status", $license_data->license);
			delete_transient("{$class}_license_message");
			if (!empty($response_key))
				delete_transient($response_key);
			wp_cache_flush();
		}
	...
	public function deactivate($key = false, $product = false, $name = false, $class = false, $url = false, $response_key = false) {
		global $thesis;
		$response = $thesis->api->post($thesis->store, array(
		...
		if (is_wp_error($response) || wp_remote_retrieve_response_code($response) !== 200) {
			...
			if ($license_data && $license_data->license == 'deactivated') {
				delete_option("{$class}_license_key");
				delete_option("{$class}_license_key_status");
				delete_transient("{$class}_license_message");
				if (!empty($response_key))
					delete_transient($response_key);
			}
	...
	public function check_license($key = false, $product = false, $name = false, $class = false, $url = false, $response_key = false) {
		global $thesis;
		$response = $thesis->api->post($thesis->store, array(
		...
		if (is_wp_error($response) || wp_remote_retrieve_response_code($response) !== 200) {
			$message = is_wp_error($response) ?
				$response->get_error_message() :
				"<span><strong>Status:</strong> ". __('Unknown', 'thesis'). "</span>";
			wp_redirect(add_query_arg(array(
				'sl_theme_activation' => 'false',
				'message' => urlencode($message)),
			...
			if (!isset($license_data->license))
				return "<span><strong>Status:</strong> ". __('Unknown', 'thesis'). "</span>";
			if ($license_data && isset($license_data->license))
				update_option("{$class}_license_key_status", $license_data->license);
			// Get expire date
			...
			// If unlimited
			if ($license_limit == 0)
				$license_limit = __('unlimited', 'thesis');
			if ($license_data->license == 'valid') {
				$message = "<span><strong>Status:</strong> ". __('Active', 'thesis'). "</span>";
				if (isset($expires) && $expires != 'lifetime')
					$message .= "<span><strong>Term:</strong> ". sprintf(__('Expires on %s', 'thesis'), $expires). "</span>";
				if (isset($expires) && $expires == 'lifetime')
					$message .= "<span><strong>Term:</strong> ". __('Lifetime', 'thesis'). "</span>";
				if ($site_count && $license_limit)
					$message .= "<span><strong>Domains:</strong> ". sprintf(__('%1$s/%2$s', 'thesis'), $site_count, $license_limit). "</span>";
			}
			elseif ($license_data->license == 'expired')
				$message = "<span><strong>Term:</strong> ". ($expires ?
					sprintf(__('Expired on %s', 'thesis'), $expires) :
					__('Expired', 'thesis')). "</span>";
			elseif ($license_data->license == 'invalid')
				$message = "<span><strong>Status:</strong> ". __('Invalid key!', 'thesis'). "</span>";
			elseif ($license_data->license == 'inactive')
				$message = "<span><strong>Status:</strong> ". __('Inactive', 'thesis'). "</span>";
			elseif ($license_data->license == 'disabled')
				$message = "<span><strong>Status:</strong> ". __('Disabled', 'thesis'). "</span>";
			elseif ($license_data->license == 'site_inactive')
				$message = "<span><strong>Status:</strong> ". __('Site is inactive', 'thesis'). "</span>";
			else
				$message = "<span><strong>Status:</strong> ". __('Unknown', 'thesis'). "</span>";
		}
		return $message;
	}
...
/*
	...
	This method's existence is dubious, as access to CSS Variable values
	probably should not be necessary outside normally-defined areas.
*/
...
*/
	public function popup($args = array()) {
		...
		return
			...
			"$tab\t\t\t<span class=\"t_popup_close\" data-style=\"close\" title=\"". __('close popup', 'thesis'). "\">&times;</span>\n".
	...
	private function strings() {
		$this->base = array(
	 	...
	 	return array(
			...
			'tracking_scripts' => 'Footer Scripts',
			'saved' => 'saved',
			'not_saved' => 'not saved',
...
/*---:[ Backwards hook compatibility ]:---*/
	...
	private function deprecated_hooks() {
		...
		add_action('hook_top_body', array($this, 'hook_before_html'));
		add_action('hook_top_body', array($this, 'thesis_hook_before_html'));
		add_action('hook_bottom_body', array($this, 'hook_after_html'));
		add_action('hook_bottom_body', array($this, 'thesis_hook_after_html'));
		add_action('hook_before_post_box', array($this, 'hook_before_thesis_post_box'));
		add_action('hook_top_post_box', array($this, 'hook_top_thesis_post_box'));
		add_action('hook_bottom_post_box', array($this, 'hook_bottom_thesis_post_box'));
		add_action('hook_after_post_box', array($this, 'hook_after_thesis_post_box'));
		add_action('hook_before_post_list', array($this, 'hook_before_thesis_post_list'));
		add_action('hook_top_post_list', array($this, 'hook_top_thesis_post_list'));
		add_action('hook_bottom_post_list', array($this, 'hook_bottom_thesis_post_list'));
		add_action('hook_after_post_list', array($this, 'hook_after_thesis_post_list'));
		add_action('hook_before_post_content', array($this, 'hook_before_thesis_post_content'));
		add_action('hook_top_post_content', array($this, 'hook_top_thesis_post_content'));
		add_action('hook_bottom_post_content', array($this, 'hook_bottom_thesis_post_content'));
		add_action('hook_after_post_content', array($this, 'hook_after_thesis_post_content'));
	}
	...
	public function thesis_hook_before_html() {
	...
	public function thesis_hook_after_html() {
		do_action('thesis_hook_after_html');
	}

	public function hook_before_thesis_post_box() {
		do_action('hook_before_thesis_post_box');
	}

	public function hook_top_thesis_post_box() {
		do_action('hook_top_thesis_post_box');
	}

	public function hook_bottom_thesis_post_box() {
		do_action('hook_bottom_thesis_post_box');
	}

	public function hook_after_thesis_post_box() {
		do_action('hook_after_thesis_post_box');
	}

	public function hook_before_thesis_post_list() {
		do_action('hook_before_thesis_post_list');
	}

	public function hook_top_thesis_post_list() {
		do_action('hook_top_thesis_post_list');
	}

	public function hook_bottom_thesis_post_list() {
		do_action('hook_bottom_thesis_post_list');
	}

	public function hook_after_thesis_post_list() {
		do_action('hook_after_thesis_post_list');
	}

	public function hook_before_thesis_post_content() {
		do_action('hook_before_thesis_post_content');
	}

	public function hook_top_thesis_post_content() {
		do_action('hook_top_thesis_post_content');
	}

	public function hook_bottom_thesis_post_content() {
		do_action('hook_bottom_thesis_post_content');
	}

	public function hook_after_thesis_post_content() {
		do_action('hook_after_thesis_post_content');
	}
