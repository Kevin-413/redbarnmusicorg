...
class thesis_form_api {
	public function fields($fields, $values = array(), $id_prefix = false, $name_prefix = false, $tabindex = 1, $depth = 0) {
		...
		foreach ($fields as $id => $field)
			if (!empty($field['type'])) {
				...
				$tabindex = $form_fields['tabindex'] = $form_field['tabindex'];
			}
		return $form_fields;
	}
	...
	public function field_group($field, $values = array(), $id_prefix = false, $name_prefix = false, $tabindex = 1, $depth = 0) {
		...
		$group = $this->fields($field['fields'], $values, $id_prefix, $name_prefix, $tabindex, !empty($field['label']) ? $depth + 2 : $depth + 1);
		$group['output'] =
			"$tab<div class=\"$class\">\n".
			(!empty($field['label']) ?
			"$tab\t<label>{$field['label']} <span class=\"toggle_group\" data-style=\"toggle\" title=\"". __('show/hide options', 'thesis'). "\">&#8862;</span></label>\n".
			"$tab\t<div class=\"group_fields\">\n" : '').
			(!empty($field['description']) ?
			"$tab\t\t<div class=\"option_item option_field group_description\">{$field['description']}</div>\n" : '').
			$group['output'].
			(!empty($field['label']) ?
			"$tab\t</div>\n" : '').
			"$tab</div>\n";
	...
	public function field_object_set($field, $values = array(), $id_prefix = false, $name_prefix = false, $tabindex = 1, $depth = 0) {
		...
		foreach ($active as $id => $label)
			$list .= "$tab\t\t<li class=\"option_object\" data-style=\"button object\" id=\"object_$id\" data-id=\"$id\" data-tooltip=\"". esc_attr($tips[$id]). "\" title=\"". __('edit this object', 'thesis'). "\">$label</li>\n";
		return array(
			'output' =>
				"$tab<div class=\"$class\">\n".
	...
	public function object_popup($args = array()) {
		...
		return
			...
			"$tab\t\t<span class=\"object_popup_close\" data-style=\"close\" title=\"". __('close popup', 'thesis'). "\">&times;</span>\n".
	...
	public function field($field, $id, $name, $value = false, $tabindex = 1, $depth = 0) {
		...
		// Read-only value override
		if (!empty($field['readonly']) && !empty($field['default']))
			$value = $field['default'];
		if ($field['type'] == 'checkbox')
		...
		if ($field['type'] == 'text') {
			...
			$output =
				...
				"$tab\t\t<input type=\"$type\" class=\"text_input$class\" id=\"$id\" name=\"$name\" value=\"". esc_attr($value). "\"$placeholder$readonly$req tabindex=\"$tabindex\" />\n".
				(!empty($field['counter']) ?
