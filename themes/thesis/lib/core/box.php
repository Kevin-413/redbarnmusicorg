...
/*
	TODO: $ignore_display is no longer used (r1930)
*/
	...
	public function _html_admin() {
		...
		$html['_id'] = !empty($this->name) || !empty($this->_parent) ? array_merge(array(
			...
			'tooltip' => sprintf(__('Specify a Display ID here, and you&#8217;ll be able to target this Box via <a href="%1$s">Skin Display Options</a> and also <a href="%2$s">hooks in your custom %3$s</a>.', 'thesis'), 'https://diythemes.com/thesis/rtfm/api/skin/display-options/', 'https://diythemes.com/thesis/rtfm/api/hooks/', $thesis->api->base['php']),
			'description' => sprintf(__('Display ID syntax: %s_{ID}', 'thesis'), $this->_class)) : array(
			'label' => __('Display ID', 'thesis'),
			'tooltip' => sprintf(__('Specify a Display ID here, and you&#8217;ll be able to control this Box with <a href="%s">Skin Display Options</a>.', 'thesis'), 'https://diythemes.com/thesis/rtfm/api/skin/display-options/'),
	...
	public function _class_canvas() {
		global $thesis;
		$options = $thesis->api->form->fields($this->_class_options(), $this->class_options, '', $this->_class, 10, 3);
...
*/
	...
	public function _instance_canvas() {
		global $thesis;
		$options = $thesis->api->form->fields($this->_options(), $this->options, '', "{$this->_class}[$this->_id]", 10, 3);
