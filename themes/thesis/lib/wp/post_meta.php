...
class thesis_post_meta {
	...
	public function __construct($id, $options, $tabindex) {
		...
		$this->post_meta['exclude'] = !empty($this->post_meta['exclude']) && is_array($this->post_meta['exclude']) ?
			$this->post_meta['exclude'] : array();
	...
	public function save() {
		...
		if (is_array($save = $thesis->api->set_options($this->post_meta['fields'], !empty($_POST[$this->id]) ? $_POST[$this->id] : false, "{$this->id}_", 'default', $post->ID)))
