...
class thesis_search_widget extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		$type = !empty($instance['show_submit']) ? 'submit' : 'hidden';
	...
	function update($new_instance, $old_instance) {
		...
		$instance['show_submit'] = !empty($new_instance['show_submit']) ? $new_instance['show_submit'] : false;
		$instance['submit_value'] = !empty($new_instance['submit_value']) ? $new_instance['submit_value'] : false;
...
class thesis_killer_recent_entries extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		while ($thesis_kre_query->have_posts()) {
			...
			$entries .=
				"<li><a href=\"". esc_url(get_permalink($thesis_kre_query->post->ID)). "\" title=\"". __('Read ', 'thesis'). $thesis->api->efh($thesis_kre_query->post->post_title, 'thesis'). "\" rel=\"bookmark\">". $thesis->api->efh($thesis_kre_query->post->post_title, 'thesis'). "</a>".
				($comms == 1 ?
...
class thesis_dashboard_rss {
	private $feed = 'https://diythemes.com/feed/';
	...
	function widget() {
		...
		if (!empty($rss_items)) {
			...
			foreach ($rss_items as $item)
				$items .= "\t\t<li><a class=\"rsswidget\" href=\"". esc_url($item->get_permalink()). "\" title=\"". trim($thesis->api->efh($item->get_description())). "\">". trim($thesis->api->efh($item->get_title())). "</a> <span class=\"rss-date\">". trim($thesis->api->efh($item->get_date($date_format))). "</span></li>\n";
		}
		else
