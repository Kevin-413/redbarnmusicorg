jQuery(document).ready(function($) {
	...
	if($inputs.length) {
		$inputs.each(function(e) {			
			jQuery(this).focus(function(f){
				...
				jQuery(this).closest('.forminator-field').find('.forminator-error-message').css({'display':'none'}); 
			});
		});
	...
	function splite_handle_successful_popup_submission() {
	...
	// Keep backward compatibility if an old Contact Form 7 shortcode is pasted into the popup content.
	document.addEventListener( 'wpcf7mailsent', function( event ) {
		splite_handle_successful_popup_submission();
	}, false );

	// Forminator can hide the form and show its own thank-you message. This observer only supports Slick Popup's optional redirect/autoclose settings after Forminator shows a success message.
	var splite_forminator_success_handled = false;
	var splite_forminator_observer_target = document.getElementById('splite_form_container');
	if (splite_forminator_observer_target) {
		var splite_forminator_observer = new MutationObserver(function() {
			if (splite_forminator_success_handled) { return; }
			var success_message = jQuery('#splite_form_container .forminator-response-message.forminator-success, #splite_form_container .forminator-success');
			if (success_message.length && success_message.is(':visible')) {
				splite_forminator_success_handled = true;
				splite_handle_successful_popup_submission();
			}
		});
		splite_forminator_observer.observe(splite_forminator_observer_target, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'style'] });
	}
