      ...
      if ( $override ) {
        ...
        foreach ( $urls_to_try as $url_base ) {
          ...
          if ( $post && !property_exists( $post, 'code' ) && $post->license === 'valid' ) {
            ...
            // The store reports a failure in one of two shapes: an `error` key for
            // activation problems (no_activations_left, expired, key_mismatch...), or
            // a plain `license` status such as site_inactive when the key is fine but
            // not valid for this install. Only reading `error` collapsed the second
            // case into 'invalid_response', which reads like a connection failure and
            // is not one, so check both before giving up.
            $status = 'no_response';
            if ( $post ) {
              if ( isset( $post->error ) ) {
                $status = $post->error;
              }
              else if ( isset( $post->license ) && is_string( $post->license ) ) {
                $status = $post->license;
              }
              else {
