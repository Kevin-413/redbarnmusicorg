...
function wpmc_scan_once_taxonomies_acf() {
    ...
    $wpmc->run_paged_parser( 'acf-taxonomy-terms', function( $offset, $limit ) use ( $wpdb ) {
        return $wpdb->get_results( $wpdb->prepare( "SELECT DISTINCT x.term_taxonomy_id, x.term_id, x.taxonomy FROM {$wpdb->term_taxonomy} x INNER JOIN {$wpdb->termmeta} y ON x.term_id = y.term_id ORDER BY x.term_taxonomy_id ASC LIMIT %d, %d", $offset, $limit ) );
    }, function( $terms ) {
        foreach ( $terms as $term ) {
            $termStr = $term->taxonomy . '_' . $term->term_id;
            $fields = get_field_objects( $termStr );
            if ( !empty( $fields ) && is_array( $fields ) ) {
                foreach ( $fields as $field ) wpmc_scan_postmeta_acf_field( $field, $termStr, 16 );
            }
        }
    }, 50 );
}
...
function wpmc_scan_html_acf( $html, $id ) {
    ...
    $images_urls = apply_filters( 'wpmc_acf_urls', $images_urls, $images_ids, $id );
    $images_ids  = apply_filters( 'wpmc_acf_ids', $images_ids, $images_urls, $id );
...
function wpmc_scan_acf_block( $data, &$images_ids, &$images_urls ) {
    ...
    if ( is_string( $data ) ) {
        ...
        $images_urls = array_merge( $images_urls, $wpmc->get_urls_from_string( $data ) );
        return;
    }
