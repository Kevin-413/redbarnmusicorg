...
class MeowPro_WPMC_Core {
	...
	private $last_file_page = array( 'scanned' => 0, 'finished' => true, 'skipped' => array() );
	...
	public function __construct( $core ) {
		...
		add_filter( 'wpmc_list_uploaded_files', array( $this, 'list_uploaded_files' ), 10, 4 );
		add_filter( 'wpmc_file_page_info', array( $this, 'file_page_info' ), 10, 1 );
	...
	public function __destruct() {
		...
		remove_filter( 'wpmc_list_uploaded_files', array( $this, 'list_uploaded_files' ), 10 );
		remove_filter( 'wpmc_file_page_info', array( $this, 'file_page_info' ), 10 );
	...
	function live_content( $post ) {
		...
		if ( get_post_field( 'post_status', $post ) == 'publish' ) {
			...
			if ( $html === false ) {
				...
				$this->core->safe_do_action( 'wpmc_scan_post', $html, $post ); // Uses same scan process as post_content. Some duplication of effort...
			}
		}
	}

	function list_uploaded_files( $result, $path, $offset = 0, $limit = -1 ) {
		$real_path = $this->core->resolve_upload_path( $path ?: '', true );
		if ( is_wp_error( $real_path ) || !is_dir( $real_path ) ) {
			throw new RuntimeException( is_wp_error( $real_path ) ? $real_path->get_error_message() : __( 'The requested uploads path is not a directory.', 'media-cleaner' ) );
		}
		$files = $this->scan_list_uploaded_files( $real_path, $offset, $limit );
		return $files;
	}

	function file_page_info( $info ) {
		return $this->last_file_page;
	}

	function scan_list_uploaded_files( $dir, $offset = 0, $limit = -1 ) {
		$result = array();
		...
		$position = 0;
		$finished = true;
		$skipped = array( 'excluded' => 0, 'filtered' => 0, 'symlink' => 0, 'unreadable' => 0 );

		try {
			$iterator = new FilesystemIterator( $dir, FilesystemIterator::SKIP_DOTS | FilesystemIterator::CURRENT_AS_FILEINFO );
		}
		catch ( UnexpectedValueException $e ) {
			throw new RuntimeException( sprintf( __( 'The uploads directory is not readable: %s', 'media-cleaner' ), $e->getMessage() ) );
		}
		...
		foreach ( $iterator as $file_info ) {
			...
			if ( in_array( $file, $this->excluded_dirs, true ) ) {
				$skipped['excluded']++;
				continue;
			}
			if ( $file_info->isLink() ) {
			...
			if ( $file_info->isDir() ) {
				...
				if ( $dirs_filter && preg_match( $dirs_filter, $relativepath_dir ) !== 1 ) {
					$skipped['filtered']++;
					continue;
				}
				if ( $thumbnails_only && !preg_match( '/-\d+x\d+\.(?:jpe?g|png|gif|webp|avif)$/i', $file ) ) {
					$skipped['filtered']++;
					continue;
				}
				if ( $files_filter ) {
					if ( @preg_match( $files_filter, $file ) === 0 ) {
						$skipped['filtered']++;
						continue;
					}
		...
		$this->last_file_page = array( 'scanned' => $scanned, 'next_cursor' => $position, 'finished' => $finished, 'skipped' => $skipped );
		return $result;
	}
	...
	function buildCroppedFileCache() {
		...
		$run_id = $this->core->get_run_id( true );
		$phase = 'croppedaliases';
		$target = 'cropped-attachment-aliases';
		$marker = $this->core->runs->get_work( $run_id, $phase, $target );
		if ( $marker && $marker->status === 'complete' ) return;
		if ( !$marker ) {
			if ( !$this->core->runs->enqueue_work( $run_id, $phase, 'initializer', $target ) ) {
				throw new RuntimeException( __( 'Media Cleaner could not initialize cropped attachment aliases.', 'media-cleaner' ) );
			}
			$marker = $this->core->runs->get_work( $run_id, $phase, $target );
		}
		// Get all rows with a -e9999999999999 timestamp in the filename.
		// Read them in batches: a site can have any number of cropped aliases, and nothing here
		// needs the whole set at once since each row only produces a reference. The ORDER BY also
		// keeps the paging stable from one batch to the next.
		$batch_size = 1000;
		$offset = 0;
		$total = 0;
		do {
			$sql = $wpdb->prepare( "SELECT meta_value,post_id FROM {$wpdb->postmeta} WHERE meta_key='_wp_attached_file' and meta_value REGEXP '-e[0-9]{13}.[A-Za-z]{3,4}' order by meta_value LIMIT %d OFFSET %d", $batch_size, $offset );
			$results = $wpdb->get_results( $sql, ARRAY_N );
			if ( $wpdb->last_error ) throw new RuntimeException( sprintf( __( 'Media Cleaner could not read cropped attachment aliases: %s', 'media-cleaner' ), $wpdb->last_error ) );
			foreach ($results as $result) {
				...
				if (preg_match('/[A-Za-z0-9-_,\)\(\\\\\/.\s](-e\d{13}).[a-z]{3,4}/', $result[0], $matches, PREG_OFFSET_CAPTURE ) ) {
					...
					$this->core->add_reference_url( $precroppedfilename, 'MEDIA LIBRARY', (int) $result[1] );
				}
			}
		...
		if ( !$marker || !$this->core->runs->update_work( $marker->id, 'complete', $total ) ) {
			throw new RuntimeException( __( 'Media Cleaner could not finalize cropped attachment aliases.', 'media-cleaner' ) );
		}
	...
	function check_file( $in_use, $path ) {
		...
		if ( $this->core->current_method === 'files' ) {
			$main_path = $this->core->clean_url_from_resolution( $clean_path );
			if ( $main_path !== $clean_path && $this->core->reference_exists( $main_path, null ) ) return true;
		}
